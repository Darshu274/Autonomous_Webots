"""
RosBot Final System - "Dynamic Retargeting & Safer Red Walls"

FIXES:
1. DYNAMIC RE-TARGETING: If a pillar position is refined by >20cm while the 
   robot is moving towards it, the robot immediately replans the path.
2. RED WALL SAFETY: 
   - Restored DEAD_END_RADIUS to 3.0m (forces robot to leave the area).
   - Lowered detection threshold to 0.05 (reacts earlier).
"""
...
AUTONOMOUS_BASE_SPEED = 12.0
AUTONOMOUS_TURN_SPEED = 5.0 
FINAL_RUN_SPEED = 15.0 
...
LIDAR_MAX_MAPPING_DISTANCE = 8.0 
...
# FIX: Lowered threshold to 5% (0.05) to detect walls earlier like previous code
RED_WALL_PIXEL_THRESHOLD = 0.2  
RED_WALL_REVERSE_TIME = 1.0      
RED_WALL_TURN_ANGLE = math.pi    
# FIX: Increased back to 3.0m to effectively skip the entire bad area
DEAD_END_RADIUS_METERS = 2.0     
RED_WALL_PROXIMITY_CHECK_RADIUS = 3.0 

# --- Stuck Recovery Specific Parameters ---
RECOVERY_ROTATE_DURATION = 2.5 
HAZARD_AVOIDANCE_DURATION = 2.0 
...
def select_target_frontier(robot_mx, robot_my, robot_theta, frontier_clusters):
    ...
    for cluster in frontier_clusters:
        ...
        if is_frontier_near_dead_end(cx, cy):
            print(f"Skipping frontier at ({cx:.1f}, {cy:.1f}) - NEAR DEAD END")
            continue
        
        if is_frontier_surrounded_by_red_walls(cx, cy):
             print(f"Skipping frontier at ({cx:.1f}, {cy:.1f}) - RED WALL SURROUNDED")
             continue
...
def detect_and_map_poison_shape(rx, ry, rt):
    ...
    for cnt in contours:
        ...
        # 1. Enforce minimum thickness of 0.5m
        # 2. Limit maximum expansion to 2.5m
...
# --- Main Loop ---
print("System Initialized: Exploration + Poison Avoidance + Red Wall Safeguard -> Speed Run.")
...
while robot.step(TIME_STEP) != -1:
    ...
    if mission_pillars['blue']['found'] and mission_pillars['yellow']['found'] and current_robot_state < STATE_RETURNING_TO_START:
        ...
        if current_robot_state not in [STATE_CENTERING_ON_PILLAR, STATE_STUCK_RECOVERY, STATE_AVOIDING_HAZARD, STATE_AVOIDING_RED_WALL]:
    ...
    if current_robot_state == STATE_EXPLORING:
        ...
        if frontier_timer % 5 == 0:
            ...
            # FIX: Initialize Target to None every time
            target = None 
            ...
            if target:
                ...
                if current_path and len(current_path) > 1:
                    ...
                    # Skip stuck recovery, just loop again
            
            else:
                # --- NO VALID FRONTIERS FOUND (All rejected or Empty) ---
                if not (mission_pillars['blue']['found'] and mission_pillars['yellow']['found']):
                    # Pillars missing? Trigger LOST MODE
                    ...
                    if new_path:
                        ...
                        path_waypoint_index = 1
                        current_robot_state = STATE_FOLLOWING_PATH
                        is_random_searching = True # Flag: Spin on arrival
                        stuck_timer_start = robot.getTime() 
                        last_stuck_check_time = robot.getTime()
                        last_stuck_check_pos = (robot_x, robot_y)
                    else:
                    ...
                    # Mapping done, Pillars found. Just spin or wait.
                    motors_set_velocity(-AUTONOMOUS_TURN_SPEED, AUTONOMOUS_TURN_SPEED)
                    robot.step(TIME_STEP * 20)
                    motors_stop()
        frontier_timer += 1
        ...
        # --- NEW: LIVE TARGET MONITORING FOR PILLAR MOVES ---
